...
import { getProjectWithPlots, updateProject } from "@/lib/firebase-service"
import { useAuth } from "@/lib/auth-context"
import ProjectLayoutEditor from "@/components/admin/project-layout-editor"
import type { Project, Plot, GridCell } from "@/lib/models"
...
export default function EditProjectPage({ params }: { params: { projectId: string } }) {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        setPlotSizes(projectData.plotSizes || "")
        setStartingPrice(projectData.startingPrice.toString())
        setStatus(projectData.status)

        // Set grid values if available
        if (projectData.gridSize) {
          setGridSize(projectData.gridSize)
        }

        if (projectData.gridCells) {
          setGridCells(projectData.gridCells)
        } else {
          // Initialize grid from plots if available
          if (projectData.plots && projectData.plots.length > 0) {
            ...
            // Initialize grid
            const newGridSize = { rows: maxRow + 1, cols: maxCol + 1 }
            setGridSize(newGridSize)

            const newGrid = initializeEmptyGrid(newGridSize.rows, newGridSize.cols)

            // Fill grid with plot data
            projectData.plots.forEach((plot) => {
              if (plot.row < newGridSize.rows && plot.col < newGridSize.cols) {
                newGrid[plot.row][plot.col] = {
                  row: plot.row,
                  col: plot.col,
                  type: plot.type,
  ...
  // Initialize empty grid
  const initializeEmptyGrid = (rows: number, cols: number) => {
  ...
  const initializeGrid = (rows: number, cols: number) => {
    setGridCells(initializeEmptyGrid(rows, cols))
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateProject(params.projectId, projectData)
      ...
      <AppShell navItems={navItems} title="Admin Dashboard">
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Update project details and layout</p>
          </div>
        </div>
          ...
          <TabsContent value="details">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Update the basic details of your real estate project</CardDescription>
              </CardHeader>
          ...
          <TabsContent value="layout">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Update the plot layout for your project</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
